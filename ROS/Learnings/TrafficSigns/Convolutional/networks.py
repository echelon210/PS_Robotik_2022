...
from preprocessing.preprocessing import augmentations_basic_noise
from preprocessing.preprocessing import lazy_load_test_batches_flow
...
def build_convolutional_model(layer_list, params=parameter, custom_layer=False):
    # Architecture
    model = Sequential([])
    ...
    if custom_layer:
        ...
        model.add(layers.Lambda(costum_activation, name="activation_layer"))
    else:
        model.add(layers.Dense(params['num_classes'], activation="softmax"))
...
def costum_activation(tensor):
    activated_node = tf.nn.sigmoid(tensor[:, :1])
    # activated_node_boundary = tf.nn.l2_loss(tensor[:, 1:])
    # return tf.concat([activated_node, activated_node_boundary], axis=1)
...
def train_model(train, test, model, params, save_model_path, save_evaluation_path):
    Xtrain_path, y_train = train
    Xtest_path, y_test = test

    for i in range(params['epochs']):
        train_gen = lazy_load_test_batches_flow(Xtrain_path, y_train,
                                               batch_size=params["batch_size"],
                                               target_size=list(params['input_shape'])[:2],
                                               augmentation_list=augmentations_basic_noise)

        test_gen = lazy_load_test_batches_flow(Xtest_path, y_test,
                                               batch_size=params["batch_size"],
                                               target_size=list(params['input_shape'])[:2],
                                               augmentation_list=augmentations_basic_noise)

        print("Train: {}".format(i))
        model.fit(train_gen, batch_size=params['batch_size'], epochs=1)
        model_evaluation(test_gen, model, params, i, save_model_path, save_evaluation_path)
