...
from image_preperation import load_image
...
def target_selection(X_path, y_t, lmax_images, target_size):
    rand_perm = np.random.permutation(len(X_path))

    if len(X_path) > lmax_images:
        rand_choices = np.random.choice(rand_perm, lmax_images)
    else:
        rand_choices = np.random.choice(rand_perm, len(X_path))

    X, y = np.empty((0, target_size[0], target_size[1])), np.empty((0, 5))
    for choice in rand_choices:
        img = load_image(X_path[choice])
        
        # resize image and convert to greyscale
        image = tf.image.resize(tf.image.rgb_to_grayscale(img), list(target_size))
        image = tf.cast(image,'int64')
        X = np.append(X, tf.reshape(image, (1, target_size[0], target_size[1])).numpy(), axis=0)
        y = np.append(y, np.reshape(y_t[choice, :], (1, 5)), axis=0)

    return X.reshape((-1, target_size[1], target_size[1], 1)), y


def lazy_load_test_batches_flow(X_path, y, lmax_images=128, batch_size=32,
                                target_size=(150, 150),
                                color_mode='grayscale',
                                augmentation_list=None):

    X, y = target_selection(X_path, y, lmax_images, target_size)
    config = _augmentation_config(augmentation_list)
